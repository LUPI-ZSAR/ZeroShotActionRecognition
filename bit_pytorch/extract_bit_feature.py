...
def gather_data(args, video_list):
  ...
  for item in video_list:
    ...
    logits = logits[0]
    idxs = np.argsort(-logits)[:5]
    # outs[name] = [idxs.tolist(), sigmoid(logits[idxs]).tolist()]
    outs[name] = []
    for idx in idxs[:5]:
      # outs[name].append((sigmoid(logits[idx]), labels[idx]))
      outs[name].append(labels[idx])

  print(f'Gathered %d data' % (len(outs)))

  json.dump(outs, open(os.path.join(args.output_dir, '..', 
    '%s_top5_preds_olympic.json'%(args.model.lower())), 'w'))



parser = argparse.ArgumentParser()
parser.add_argument('--model',default='BiT-M-R50x1', type=str)
parser.add_argument('--model_dir',default= './datasets/premodels/BiT/',type=str)
parser.add_argument('--video_dir', default='./video',type=str)
parser.add_argument('--video_meta_file',default='./datasets/{}_video_metas.jsonl',type=str)
parser.add_argument('--num_frames_per_video', type=int, default=2)
parser.add_argument('--output_dir', default= './datasets//feature',type=str)
...
def main():
  ...
  if len(todo_video_list) > 0:
    ...
    progress_bar = ProgressBar(maxval=len(todo_video_list))
    progress_bar.start()
